...
module.exports = {
  getDashboard: async (req, res) => {
    try {
      const thoughts = await thoughtDiary
        .find({ user: req.user.id })
        .sort({ _id: -1 });
      console.log("thoughts order" + thoughts);
      const users = await userDetails.find();
      const motivations = await motivationsQuotes.find({
        id: req.params.id,
        like: true,
      });
  ...
  getChartData: async (req, res) => {
    try {
      ...
      const data = await thoughtDiary
        .find({ id: req.params.id, user: req.user.id })
        .limit(7);
  ...
  getTypesOfEmotion: async (req, res) => {
    try {
      const limitNumber = 7;
      const great = await thoughtDiary
        .find({ category: "Great" })
        .limit(limitNumber);
      const veryGood = await thoughtDiary
        .find({ category: "Very Good" })
        .limit(limitNumber);
      const good = await thoughtDiary
        .find({ category: "Good" })
        .limit(limitNumber);
  ...
  dislikeQuote: async (req, res) => {
    ...
    try {
      await motivationsQuotes.findByIdAndUpdate(req.body.id, {
        like: false,
      });
